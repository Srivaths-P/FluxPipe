...
class OpenEMS:
    ...
    def run_openems(self, options='view solve', z=50, initialize=True, show_plot=True, numThreads=8):
        ...
        sim_name_basename = os.path.basename(self.name)
        safe_sim_name_component = "".join(c if c.isalnum() or c in ('_', '-') else '_' for c in sim_name_basename)

        base_temp_dir = tempfile.gettempdir()
        simpath_parent_dir = os.path.join(base_temp_dir,
                                          'pyopenems_simulations')  # A general folder for all pyopenems sims

        if not os.path.exists(simpath_parent_dir):
            try:
                os.makedirs(simpath_parent_dir)
            except OSError as e:
                print(f"PYOPENEMS_ERR: Could not create parent temp dir {simpath_parent_dir}: {e}")
                simpath_parent_dir = os.path.join(os.getcwd(), '.pyopenems_sim_temp')
                if not os.path.exists(simpath_parent_dir): os.makedirs(simpath_parent_dir)

        simpath = os.path.join(simpath_parent_dir, safe_sim_name_component)

        if not os.path.exists(simpath):
            try:
                os.makedirs(simpath)
            except OSError as e:
                print(f"PYOPENEMS_ERR: Could not create specific simpath {simpath}: {e}")
                simpath = tempfile.mkdtemp(prefix=f"{safe_sim_name_component}_", dir=simpath_parent_dir)
                print(f"PYOPENEMS_WARN: Using fallback simpath: {simpath}")
        ...
        if 'view' in options:
                CSX_file = os.path.join(simpath, 'csx.xml')
